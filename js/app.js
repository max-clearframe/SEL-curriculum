...
const ANIM_START = 0.47;  // book starts opening at 47% scroll
const ANIM_END   = 0.62;  // book fully open at 62% scroll
...
function animateHero() {
  ...
  const freeLessonLink = document.querySelector('.hero-free-lesson-link');
...
function setupSectionAnimation(section) {
  ...
  const midpoint = ((enter + leave) / 2) * 700; // in vh units
...
/* ── Contact form: Formspree AJAX + PDF download ─────────────── */
(function () {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Trigger PDF download — drop the real file at assets/free-lesson.pdf when ready
        const a = document.createElement('a');
        a.href = '/assets/free-lesson.pdf';
        a.download = 'Clearframe-Free-Lesson.pdf';
      ...
      submitBtn.innerHTML = 'Send message <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 12h14M12 5l7 7-7 7" stroke-linecap="round" stroke-linejoin="round"/></svg>';
    }
  });
})();
